...
def ThrustcalcV2(V0,P):
    ...
    for i in PitchRange:
        ...
        while Iterating:
            ...
            VE = np.sqrt((w + V0) ** 2 + Propspeed ** 2)
            Fw = 8 * np.pi * r / Nprop / c * w - VE / (w + V0) * (ClProp * Propspeed - CdProp * (w + V0))
            Fwaccent = 8 * np.pi * r / Nprop / c - ClProp * Propspeed * (1 / VE - VE / (V0 + w) ** 2) + CdProp * (
            w + V0) / VE
        ...
        if Power >= P and Power <= P+25000 and running:
            running = False
            Final = [efflist[-1], Thrustlst[-1], i]
        efflist.append(etaP)
        Thrustlst.append(Thrust)
...
def Powercalc(V0, T):
    ...
    for i in PitchRange:
        ...
        if Thrust >= T and Thrust <= T+100 and running:
            running = False
            Final = [efflist[-1], Thrustlst[-1], i, Powerlst[-1]]
        efflist.append(etaP)
        Thrustlst.append(Thrust)
...
def initialise_propinertia_x(inp):
    global ixg
    ixg = inp


def initialise_propinertia_z(inp):
    global izg
    izg = inp
