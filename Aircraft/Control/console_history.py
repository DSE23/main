...
V = V_inf
def trimlift(angles):
    alpha = angles[0] * Q_("rad")
    de = angles[1] * Q_("deg")
    Cl, Cd, Cm, xcp = lookup_data(alpha, 0.2,0.)
    Clh, Cdh, Cmh, xcph = lookup_data(alpha, 0.5, de)
    Cn_w = -Cl*np.cos(alpha) - Cd*np.sin(alpha)
    Cn_h = -Clh*np.cos(alpha) - Cdh*np.sin(alpha)
    q_dp = 0.5*rho*V**2
    L_w = Cn_w * q_dp * S_w
    L_h = Cn_h * q_dp * S_h
    F_z = W*np.cos(alpha)  + L_w + L_h
    x_w = xlemac + 0.25 * MAC
    x_h = X_h + 0.25 * MAC_htail
    M_y = L_w * ((X_w+0.25*MAC)-xcg) + Cm * q_dp * S_w * MAC + Cmh * q_dp * S_h * MAC_htail +\
          L_h * (l_h - xcg)
    print(F_z.magnitude**2 + M_y.magnitude**2)
    return F_z.magnitude**2+M_y.magnitude**2      

def trim():
    
    initial_guess = [0.02, -2]
    angle_bound = optimize.Bounds(-30,30)
    result = optimize.minimize(trimlift, initial_guess, method='POWELL', bounds=angle_bound, options={'ftol': 0.001, 'xtol':0.00001})
...
disc_wing_h = np.zeros((len(khlst)-1, 10))
  # set aileron for negative stations
...
for t_current in tlst:
    ...
    for i in range(0, len(kwlst)-1):
        ...
        for j in range(5):
            ...
            Cl, dummy, dummy, dummy = lookup_data(alpha_e, ca_c, da_local)
            alpha_i = Cl / (m.pi * AR_w * e_w)
        ...
        disc_wing_w[i,4] = (Ft_w_d*y_i).magnitude
        disc_wing_w[i,5] = Sloc.magnitude
        disc_wing_w[i,6] = downwash_angle
        disc_wing_w[i,7] = y_i.magnitude
        
    downwash_func = interpolate.interp1d(disc_wing_w[:,7],disc_wing_w[:,6])
...
#    Ft_w = 0.5 * rho * V_local ** 2 * S_w * Ct_w
    ...
    #Horizontal Tail
    average_distance = b_h/4
    average_downwash = downwash_func(average_distance)
    alpha_h = alpha_nose
    dx_h = l_h
    ...
    for i in range(0, len(khlst)-1):
        de_local = de                        # Local aileron deflection of piece
        ...
        roll_induced_alpha = p * y_i / V_local
        downwash = downwash_func(y_i)
        alpha_h = alpha_nose + roll_induced_alpha - downwash
        alpha_i = 0.
        for j in range(5):
            alpha_e = alpha_h - alpha_i
            Cl, dummy, dummy, dummy = lookup_data_emp(alpha_e,ce_c,0,data_NACA0009)
            alpha_i = Cl / (m.pi * AR_h * e_w)

        alpha_h -= alpha_i
        
        Cl, Cd, Cm, xcp = lookup_data_emp(alpha_h, ce_c, 0,data_NACA0009)
        Cn_h = -Cl*m.cos(alpha_h) - Cd*m.sin(alpha_h)
        Ct_h = Cl*m.sin(alpha_h) - Cd*m.cos(alpha_h) 
        Fn_h_d = 0.5 * rho * V_local ** 2 * Sloc * Cn_h
        Ft_h_d = 0.5 * rho * V_local ** 2 * Sloc * Ct_h
        
        disc_wing_h[i,1] = Fn_h_d.magnitude
        disc_wing_h[i,2] = Ft_h_d.magnitude
        disc_wing_h[i,3] = (Fn_h_d*y_i).magnitude
        disc_wing_h[i,4] = (Ft_h_d*y_i).magnitude
        disc_wing_h[i,5] = Sloc.magnitude
        disc_wing_h[i,6] = downwash_angle
