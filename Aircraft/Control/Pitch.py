...
q = Q_("1. 1/s")  # initial pitch rate [rad/s]
...
# for de in np.arange (-0.45, -0.25, 0.005):
#     alpha_title = alpha_nose
#     de = de
#     zz= zz+1
#     print(de)
#     # Init sim
...
for t_current in np.arange(0, (t_end).magnitude, dt.magnitude):
    ...
    for i in range(0, len(kwlst) - 1):
        ...
        delta_alpha = roll_induced_alpha  # Difference between AoA nose and piece
    ...
    for i in range(0, len(khlst) - 1):
        de_local = 0 #disc_wing_h[i][0]  # Local aileron deflection of piece
        ...
        V_local = V_inf #(V_inf + delta_V) * m.cos(beta_nose)

        # Determine change in angle of attack due to roll
        roll_induced_alpha = p * y_i / V_local
        downwash = 0 #down_wash_func(y_i)
        alpha_h = alpha_nose + roll_induced_alpha - downwash

        delta_alpha = - alpha_nose + alpha_h  # Difference between AoA nose and piece
...
# plt.figure(zz)
# plt.title((round(de,2), T.magnitude))
