...
def lookup_datanice(alpha, ca_c, da):
...
alpha_nose = 0.03  # angle of attack of nose
...
while running:
    ...
    for i in range(1, len(kwlst)):
        ...
        for j in range(10):
            ...
            Cl, Cd = lookup_datanice(alpha_e, ca_c, da_local)
            alpha_i = Cl / (math.pi * AR_w * e_w)
        alpha = alpha_w - alpha_i
        downwash_angle = 2 * Cl / (math.pi * AR_w)
        Cl, Cd = lookup_datanice(alpha, ca_c, da_local)
    ...
    print(np.degrees(disc_wing_w[:, 1]))
    for i in range(1, len(khlst)):
        ...
        Cl, Cd = lookup_datanice(alpha, 0.01, 0.)
        Lift = 0.5 * rho * V_inf ** 2 * Sloc * Cl
        Drag = 0.5 * rho * V_inf ** 2 * Sloc * Cd
    ...
    for i in range(1, len(kvlst)):
        ...
        Cl, Cd = lookup_datanice(beta, 0.01, 0.)
        Lift = 0.5 * rho * V_inf ** 2 * Sloc * Cl
        Drag = 0.5 * rho * V_inf ** 2 * Sloc * Cd
