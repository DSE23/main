...
for t_current in np.arange(0,(t_end).magnitude,dt.magnitude):
    ...
    for i in range(0, len(kwlst)-1):
        ...
        Lift_body = Lift*math.cos(delta_alpha) - Drag*math.sin(delta_alpha)
        Drag_body = Drag*math.cos(delta_alpha) + Lift*math.sin(delta_alpha)
    ...
    Fy = Sideforce*math.cos(beta_nose) - math.sin(beta_nose)*(Sum_Dw + Sum_Dh + D_fus_gear) + W*math.sin(Theta)
